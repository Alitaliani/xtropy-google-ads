...
- **Never hardcode operator emails** (e.g., `team@example.com`, `operator@example.com`) in any schedule created for an end user.
...
The operator's own weekly SEM run uses a **local stdio** MCP with `skip_drive_upload=false` + a `summary_email` value — it DOES upload to Drive and email the team. That path is operator-only and lives in `~/.claude/scheduled-tasks/weekly-sem-reports/SKILL.md` on the operator's machine. End users never hit that path.
