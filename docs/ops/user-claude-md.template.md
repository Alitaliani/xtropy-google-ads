...
Hosted at `https://mcp.trafficarchitect.com/mcp`. You have 38+ Google Ads tools and 7 methodology skills. Use them directly — no special preamble needed.
...
- One call: `run_weekly_reports(report_mode="audit", skip_inactive=true, summary_email="")`
...
Every `generate_sem_report` and `upload_report` response includes a `download_url` like `https://mcp.trafficarchitect.com/download/AccountName_2026-04-17_18-08.xlsx`.

The user fetches it with their own API key:

```bash
curl -OJ -H "Authorization: Bearer YOUR_KEY" \
  https://mcp.trafficarchitect.com/download/AccountName_2026-04-17_18-08.xlsx
```
...
{
  "_auto_download": {
    "url": "https://mcp.trafficarchitect.com/download/AccountName_2026-04-17.xlsx?token=...",
    "filename": "AccountName_2026-04-17.xlsx",
